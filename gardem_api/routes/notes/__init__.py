...
from gardem_api.routes.notes import models, schema

router = APIRouter(prefix="/v1/farms", tags=["notes"])
...
async def note_create(
    ...
    async with db() as tx:
        obj = models.Note(farm_id=farm_id, **body.model_dump())

        tx.add(obj)
        await tx.commit()
        await tx.refresh(obj)

        return schema.Note.model_validate(obj)
...
async def note_all(
    ...
    user: User = Depends(current_user),
    db: Datasource = Depends(datasource),
) -> schema.NoteListResult:
    if not user.allowed("farm_notes:list"):
        raise HTTPException(
            status.HTTP_403_FORBIDDEN, "you not have access for this resource"
        )

    sentences = [models.Note.farm_id == farm_id]

    async with db() as tx:
        stmt = select(models.Note).where(*sentences).offset(offset).limit(limit)
        stmt_count = select(func.count(models.Note.id)).where(*sentences)

        query = (await tx.execute(stmt)).scalars()
        count = (await tx.execute(stmt_count)).scalar_one()

        return schema.NoteListResult(
            count=count,
            next=None,
            previous=None,
            items=[schema.Note.model_validate(note) for note in query],
        )


@router.get("/{farm_id:int}/notes/{id:int}")
async def note_retrive(
    id: int,
    farm_id: int,
    user: User = Depends(current_user),
    db: Datasource = Depends(datasource),
    ...
    sentences = [models.Note.id == id, models.Note.farm_id == farm_id]

    async with db() as tx:
        stmt = select(models.Note).where(*sentences)
...
async def note_partial_update(
    id: int,
    farm_id: int,
    body: schema.NoteUpdatable,
    user: User = Depends(current_user),
    db: Datasource = Depends(datasource),
) -> schema.Note:
    if not user.allowed("farm_notes:retrive"):
...
async def note_update(
    ...
    async with db() as tx:
        ...
        if not obj:
            obj = models.Note(id=id, farm_id=farm_id, **body.model_dump())
            await tx.add(obj)
